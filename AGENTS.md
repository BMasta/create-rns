...
* Sable physics movement preserves deposit durability through an optional mixin that makes `DepositBlock` implement `BlockSubLevelAssemblyListener`. Movement captures raw stored state without initialization, restores the source state authoritatively at the destination, and leaves ordinary removal responsible for clearing the source. Callback matching must use immutable dimension/position/state data without retaining level references; Sable has no failure callback, so an interrupted capture stays inert until the next `beforeMove` replaces it.
* Deposit claimer instance tracking stores owner block positions keyed by sided dimension/type and resolves live mining behaviors from the owning block entity on demand. Point-based queries carry a relevant operator/deposit/player position, while area-only queries derive a representative position from the bounding box; local operating boxes must remain wholly contained in one operating space. Resolved claimers are dynamically filtered through the operating-space adapter, and lookup must prune stale positions when the block entity or behaviour is gone. Client claim outlines group nearby claims into one Create Outliner cluster per operating space so Sable never applies one sublevel pose to blocks from another space.
* Deposit operation is split by responsibility: `IDepositBlockOperator` owns geometry and vein discovery, `IDepositBlockClaimer` adds exclusive ownership, `ClaimingBehaviour` owns the authoritative claimed target group, and `MiningBehaviour` constructs and executes its process directly from the claim. Claim changes invalidate the process and initialize durability; disk and client NBT synchronize claims through the existing claimer data rather than through a separate active-target state.
* Deposit targeting uses dependency-free operating-space value types and an adapter boundary. A claimed target group contains a local/cross-sublevel flag and immutable positions from exactly one operating space, but does not cache that space's identity. The adapter also owns Manhattan and squared-distance calculations so client effects compare projected physical positions without exposing Sable types to common code. The vanilla adapter identifies one main space per dimension and preserves `BlockPos` distance behavior. When Sable is loaded, the guarded Sable adapter uses the containing sublevel's stable UUID or the dimension-specific main-space fallback and delegates distances to Sable's projection-aware API. The Sable adapter can enumerate remote tagged deposits inside a mine-head-sized transformed detection volume for enter/leave diagnostics, but these results do not become claims or process targets yet; the vanilla adapter still returns no remote candidates.
* Mine-head detection geometry is defined per `MineHeadSize` as a radial half-width, axial length, and forward center offset. One constructed AABB is shared by transformed Sable discovery and the dedicated client detection-outline renderer. Detection outlines are disabled by default and toggled locally through the client-only `/rns debug deposit_detection true|false` command without a network payload; they are independent from claim-outline wrench/target activation, remain in the mine head's local operating space so Sable can apply its live pose, and reset on disconnect rather than persisting as config.
* Sable API types must stay in `compat/sable` or Sable-gated mixins under `mixin/sable`; common signatures and static initialization may reference only mod-owned operating-space types. GameTest field and method descriptors must also avoid Sable API types because NeoForge reflects attached GameTest classes during ordinary dev launches even when Sable is absent. Sable's Companion API is embedded in its distribution rather than declared by its Modrinth POM, so Gradle exposes that embedded jar only on the compile classpath and does not package it with this mod.
* Local miners do not cache or poll their source operating-space identity. The bearing and effective mine-head tip must remain in the same operating space, so fresh local claims resolve their target space from the bearing position. Sable assembly/disassembly recreates the bearing block entity and loads the source NBT into the destination instance, so a local claim restored into an uninitialized owner is validated once on the next tick against a representative target position. Same-space claims are retained without rescanning, changed-space or empty claims are rebuilt, and compatible mining progress is restored by deposit recipe after validation. Ordinary reads on initialized miners must not trigger claim validation or rebuilding.
