...
public class DepositLocationSerialization {
    ...
    public void customDepositLocationRoundTrips(GameTestHelper helper) {
        ...
        helper.assertValueEqual(restored.getLocation(), CUSTOM_LOCATION, "custom deposit location");
        helper.assertValueEqual(restored.getTypeStr(), "custom", "custom deposit type");
        helper.succeed();
    }
    ...
    public void structureDepositLocationRoundTrips(GameTestHelper helper) {
        ...
        helper.assertValueEqual(restored.getLocation(), STRUCTURE_ORIGIN.getBlockAt(7, 62, 7), "fallback structure location");
        helper.assertValueEqual(restored.getTypeStr(), "ungenerated", "fallback structure deposit type");
        helper.succeed();
    }
