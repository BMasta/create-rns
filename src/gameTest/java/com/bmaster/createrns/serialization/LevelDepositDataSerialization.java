...
public class LevelDepositDataSerialization {
    ...
    public void levelDepositDataRoundTripsSerializableState(GameTestHelper helper) {
        ...
        var original = new DepositIndex(level);
        ...
        var serialized = original.serializeNBT();
        CodecHelper.assertValueEqual(helper, serialized.getList("found", 10).size(), 1, "serialized found deposit count");
        CodecHelper.assertValueEqual(helper, serialized.getCompound("custom").getAllKeys().size(), 1,
                "serialized custom deposit type count");
        CodecHelper.assertValueEqual(helper, serialized.getList("durabilities", 10).size(), 2,
                "serialized durability count");

        var restored = new DepositIndex(level);
        ...
        CodecHelper.assertValueEqual(helper, restoredCustomDeposits.size(), 1, "deserialized custom deposit type count");
        helper.assertTrue(
        ...
        CodecHelper.assertValueEqual(helper, restoredDurabilities.size(), 2, "deserialized durability count");
        CodecHelper.assertValueEqual(helper, restoredDurabilities.getLong(DURABILITY_POS_A), DURABILITY_A,
                "durability at first block");
        CodecHelper.assertValueEqual(helper, restoredDurabilities.getLong(DURABILITY_POS_B), DURABILITY_B,
                "durability at second block");
        helper.succeed();
    }
    ...
    private static ObjectOpenHashSet<ServerDepositLocation> foundDeposits(DepositIndex data) {
        try {
    ...
    @SuppressWarnings("unchecked")
    private static Object2LongOpenHashMap<BlockPos> depositDurabilities(DepositIndex data) {
        try {
