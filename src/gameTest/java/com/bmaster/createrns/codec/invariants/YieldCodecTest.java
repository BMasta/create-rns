...
public class YieldCodecTest {
    ...
    public void weightedItemParsesAndInitializesFromItemCandidate(GameTestHelper helper) {
        ...
        CodecHelper.assertValueEqual(helper, weightedItem.weight, 5, "weighted item weight");
    ...
    public void weightedItemInitializesFromLiveItemTags(GameTestHelper helper) {
        ...
        helper.assertTrue(isInTag(helper.getLevel().registryAccess(), weightedItem.item, PLANKS_TAG),
    ...
    public void yieldParsesWithDefaultsAndInitializesItems(GameTestHelper helper) {
        ...
        CodecHelper.assertValueEqual(helper, yield.items.size(), 1, "yield weighted item count");
        CodecHelper.assertValueEqual(helper, yield.slotColor, 0, "yield slot color");
        CodecHelper.assertValueEqual(helper, yield.getCRSIds().size(), 1, "yield catalyst count");
        CodecHelper.assertValueEqual(helper, yield.getCRSIds().get(0), CreateRNS.asResource("overclock"),
                "yield catalyst id");
        ...
        CodecHelper.assertSame(helper, Items.DIAMOND, yield.items.get(0).item, "initialized yield item");
        helper.succeed();
    }
    ...
    public void yieldRetainsResolvableWeightedItemsWhenOthersFail(GameTestHelper helper) {
        ...
        CodecHelper.assertValueEqual(helper, yield.items.size(), 1, "remaining weighted item count");
        helper.assertTrue(isInTag(helper.getLevel().registryAccess(), yield.items.get(0).item, PLANKS_TAG),
                "Remaining weighted item should come from the live planks tag");
        helper.succeed();
    }

    @GameTest(template = "empty16x16")
    public void yieldRejectsUnknownCatalystDuringInitialization(GameTestHelper helper) {
        var yield = CodecHelper.assertParses(helper, Yield.CODEC, CodecHelper.json(), """
                        {
                          "items": [
                            {
        ...
        try (var logs = LogCapture.capture(CreateRNS.LOGGER.getName())) {
            ...
            helper.assertTrue(logs.contains("Failed to get element create_rns:missing_catalyst"),
