...
public class ItemWithFallbacksCodecTest {
    ...
    public void parsesSingleItemAndRoundTripsAsScalar(GameTestHelper helper) {
        ...
        var diamond = ForgeRegistries.ITEMS.getValue(ResourceLocation.parse("minecraft:diamond"));
        ...
        CodecHelper.assertSame(helper, diamond, parsed.item, "resolved fallback item");
    ...
    public void parsesSingleTagAndRoundTripsAsSingletonList(GameTestHelper helper) {
        ...
        helper.assertTrue(isInTag(helper.getLevel().registryAccess(), parsed.item, PLANKS_TAG),
    ...
    @GameTest(template = "empty16x16")
    public void codecRoundTripsOriginalEntriesAndListShape(GameTestHelper helper) {
        var codec = ItemWithFallbacks.STRICT_CODEC;
        var parsed = CodecHelper.assertParses(helper, codec, CodecHelper.json(), """
                        ...
                        """, "codec round-trip item fallback");

        var restored = roundTrip(ItemWithFallbacks.LENIENT_CODEC, parsed);

        CodecHelper.assertSame(helper, Items.AIR, restored.item,
                "Codec round-trip should preserve unresolved tag-backed fallbacks before initialization");
        ...
        helper.assertTrue(isInTag(helper.getLevel().registryAccess(), restored.item, PLANKS_TAG),
                "Codec round-trip should preserve the resolved main item");
    ...
    private static void assertRoundTrips(
        ...
        CodecHelper.assertValueEqual(helper, encoded, CodecHelper.parseJson(sourceJson), valueName + " JSON");
    }
    ...
    private static <T> T roundTrip(Codec<T> codec, T value) {
        var encoded = codec.encodeStart(NbtOps.INSTANCE, value).result().orElseThrow();
