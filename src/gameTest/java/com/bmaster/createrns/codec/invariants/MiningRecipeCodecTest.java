...
public class MiningRecipeCodecTest {
    ...
    public void parsesRecipeWithReplacementAndDurabilityDefaults(GameTestHelper helper) {
        ...
        CodecHelper.assertSame(helper, Items.DIAMOND, recipe.getYields().getFirst().items.getFirst().item,
                "resolved yield item");
        helper.succeed();
    }
    ...
    public void initializeFiltersInvalidYieldsButKeepsResolvableTagBackedYields(GameTestHelper helper) {
        ...
        var resolvedItem = recipe.getYields().getFirst().items.getFirst().item;
        helper.assertTrue(BuiltInRegistries.ITEM.wrapAsHolder(resolvedItem).is(PLANKS_TAG),
