...
public class BasicMiningRecipeCategory extends MiningRecipeCategory<BasicMiningRecipe> {
    public static final RecipeType<RecipeHolder<BasicMiningRecipe>> JEI_RECIPE_TYPE = RecipeType.createRecipeHolderType(
            ResourceLocation.fromNamespaceAndPath(CreateRNS.MOD_ID, MiningLevel.BASIC.getRecipeID()));

    private final AnimatedMiner miner = new AnimatedMiner(RNSContent.MINER_MK1_BLOCK.get(), RNSContent.MINER_MK1_DRILL);
    private static final Info<BasicMiningRecipe> info = new Info<BasicMiningRecipe>(
