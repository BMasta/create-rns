...
public class MiningRecipeHelper {
    ...
    public record SerializedRecipe(Block depositBlock, Block replacementBlock,
                                   MiningRecipe.Durability durability, int tier,
                                   List<MiningRecipe.YieldType> yield, MiningRecipe.Byproduct byproduct) {
        public static SerializedRecipe fromRecipe(MiningRecipe recipe) {
            return new SerializedRecipe(
                    ...
                    recipe.getTier(),
                    List.copyOf(recipe.getYield().types),
                    recipe.getByproduct()
            );
        }

        public MiningRecipe toRecipe(ResourceLocation id) {
            return new MiningRecipe(id, depositBlock, replacementBlock, durability, tier,
                    yield, byproduct);
        }
    }
}
