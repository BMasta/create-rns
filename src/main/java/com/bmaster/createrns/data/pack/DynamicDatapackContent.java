        ...
        return () -> {
            ...
            for (var r : recipes) {
                ...
                if (dim != DepositDimension.OVERWORLD) {
                    root.addProperty("dimension", dim.levelDimension().location().toString());
                }
                ...
                for (var yield : r.recipe().yields()) {
                    ...
                    for (var item : yield.items()) {
                        ...
                        if (candidateIds.size() == 1) {
                            itemJson.addProperty("item", candidateIds.getFirst());
                        } else {
        ...
        return () -> {
            ...
            for (var def : specEntries) {
                ...
                if (scannerIconCandidates.size() == 1) {
                    root.addProperty("scanner_icon_item", scannerIconCandidates.getFirst());
                } else {
                ...
                if (mapIconCandidates.size() == 1) {
                    root.addProperty("map_icon_item", mapIconCandidates.getFirst());
                } else {
                    var candidates = new JsonArray();
                ...
                if (def.dimension() != DepositDimension.OVERWORLD) {
                    root.addProperty("dimension", def.dimension().levelDimension().location().toString());
                }
