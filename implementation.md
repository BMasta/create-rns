...
* Core behavior: `RNSKubeJSAssembler` is the central authority for that structure-set ownership state. It replays the startup KubeJS structure-set event on demand, caches the selected structure ids plus derived enabled deposit blocks, and exposes those derived sets to always-loaded runtime code through a KubeJS-free bridge.
