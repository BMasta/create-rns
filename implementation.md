...
* Core behavior: deposit support data now comes from a separate `StartupEvents.rnsDepositStructures(...)` event. Each structure entry owns its own structure id, referenced deposit block id, scanner/map icon candidates, placement settings, weighted templates, processor-list output, and scanner-visible structure name.
