...
* System interaction: if `StartupEvents.createRnsStructureSet(...)` has no listeners, structure-set files are still left to normal datapack authoring. If listeners are present, only the dimensions whose builders are explicitly requested emit replacement overworld or nether structure-set files, but the shared scanner-selection data and built-in `deposit_spec` overrides are still regenerated from the selected structure list so omitted deposits cannot remain scannable through built-in defaults.
