...
## KubeJS Deposit Block Builder
* Player perspective: when KubeJS is installed, startup scripts can register a deposit block with `event.create('<name>', 'create_rns:deposit')` instead of rebuilding the shared deposit block boilerplate by hand.
* Player perspective: the created block behaves as a real deposit block, so systems that key off the deposit block class or `#create_rns:deposit_blocks` tag continue to recognize it. Deposit structures are now defined separately, so one deposit block may back multiple deposit worldgen entries.
* Core behavior: the custom builder type constructs `DepositBlock`, not a generic KubeJS block, and `DepositBlock` itself owns the shared default deposit properties: raw-iron-based properties, deepslate sound, high hardness/resistance, no loot table, and required-tool mining.
* Core behavior: code-registered and KubeJS-registered deposits both rely on that same `DepositBlock` default-property preset, while registration layers still inject the block tags `#create_rns:deposit_blocks`, `#minecraft:mineable/pickaxe`, and `#minecraft:needs_diamond_tool`.
* Core behavior: deposit support data now comes from a separate `StartupEvents.rnsDepositStructures(...)` event. Each structure entry owns its own structure id, referenced deposit block id, biome selector, scanner/map icon candidates, placement settings, weighted templates, processor-list output, and scanner-visible structure name. KubeJS structures default to the dimension's built-in deposit biome tag, but may target one explicit biome or biome tag; an explicit selector remains stable if the structure is later assigned to another dimension. Direct vanilla biome ids are validated during startup script execution, while modded biome ids and tags remain deferred to datapack loading because their registries do not exist at startup-script time.
* Core behavior: the structure builder exposes `preset(...)` for concise deposit setup. `overworld_common`, `overworld_uncommon`, and `overworld_rare` mirror the built-in overworld structure presets from `RNSDeposits`, while `nether_common`, `nether_uncommon`, and `nether_rare` apply the corresponding weight, height, and template mix presets used by the built-in nether deposits. Presets no longer choose a placement family by themselves.
* Core behavior: the structure builder also exposes `scannerIconMetal(...)`, `scannerIconGem(...)`, and `scannerIconDust(...)` as shorthand for the same scanner-icon fallback candidate lists used by the built-in `RNSDeposits` helpers.
* Core behavior: `event.tweak('<built-in structure id>')` starts from the registered built-in structure and deposit-spec values, then emits KubeJS data overrides for fields changed by the startup script. Single-value height and weight calls use last-write-wins behavior; the first scanner-icon, map-icon, or NBT call replaces its inherited list and later calls append ordered fallbacks/templates. `preset(...)` replaces height, weight, and the full NBT list.
* Edge behavior: tweaks reject unknown ids, duplicate tweaks, attempts to create a custom structure with a built-in id, and attempts to change a built-in structure's deposit block. Unchanged values, the structure dimension, and existing scannability are preserved unless the structure-set event separately takes ownership of that dimension.
* System interaction: a tweaked weight becomes the default weight seen by `rnsEnableDeposits`; an explicit weight on that event's `.deposit(...)` call still wins. Without deposit-enablement ownership, a weight tweak regenerates only the affected default structure set while preserving all currently enabled built-in entries and placement defaults.
* Data and assets: built-in tweaks emit KubeJS overrides for the existing deposit spec, processor list, and structure resource ids, so no generated source assets are edited. Default preset geometry, weights, templates, processor input, and scanner-icon helper lists are shared by `RNSDeposits` and the KubeJS builder through the core datapack builders.
* Maintenance invariant: built-in registration and KubeJS tweak inheritance must continue to use the registered `DepositStructureBuilder` and `DepositSpecBuilder` entries as their source of truth; repeated list methods must never accidentally retain inherited candidates after the first explicit call.
* Core behavior: deposit mining recipes may now be authored from `ServerEvents.recipes(...)` through a custom `create_rns:mining` recipe schema that exposes callback-style nested yield builders mirroring the code-side `MiningRecipeBuilder` and `YieldBuilder` flow. The mined deposit block is configured explicitly through `.block(...)`, keeping it separate from the recipe id, and must belong to `#create_rns:deposit_blocks`; the mining codec enforces the same invariant for KubeJS and raw datapack recipes. Yield catalyst ids are registry-backed references, so malformed or unknown catalysts reject the containing recipe during decoding instead of silently discarding its yield during later initialization. Yield slot colors may be authored either as raw ARGB integers or as `#rrggbb` shorthand strings. Required `.item(...)` candidate groups containing only direct item ids are validated by the mining recipe codec against the completed item registry; if none exists, KubeJS reports and skips the individual recipe without failing the datapack reload, while `.compatItem(...)` remains optional and tag-containing groups remain deferred until live tag resolution. Every JavaScript-visible recipe method captures the first Rhino statement as the fallback error location, then refines Rhino's outer-statement position against the loaded server script, so omitted required block or yield data points to the beginning of the recipe expression. Invalid block, durability, chance, and catalyst calls replace that fallback with their own call line; malformed block and catalyst values, out-of-range chance values, and non-positive item weights are retained for normal recipe decoding rather than throwing from the event callback, which prevents secondary errors from partially built recipes. The KubeJS chance and item-weight schemas remain range-constrained, while builder-authored invalid values are preserved only for the authoritative mining codec to report and reject. If script source is unavailable, attribution falls back to Rhino's coarse statement position. Item calls, including non-positive weight errors, retain Rhino's callback-level attribution because repeated calls inside one yield cannot be distinguished from the interpreter stack alone.
* Core behavior: `StartupEvents.rnsCatalysts(...)` exposes a catalyst builder that serializes catalyst registry entries into `create_rns/catalyst/*.json`, can emit the matching catalyst name/description lang keys when those strings are provided inline, and can also emit a merged `#create_rns:miner_attachments` block tag for KubeJS-authored attachment catalysts.
* Core behavior: the catalyst builder currently covers the existing datapack requirement kinds directly through `.fluid(...)` and `.attachment(...)`, with repeated calls appending more tagged requirement entries to the same catalyst definition. KubeJS attachment requirements accept explicit block ids or block-id lists only; tag-based attachment matching remains available in raw datapack JSON.
* Core behavior: `hide_if_present` uses registry-backed catalyst holders, so missing targets invalidate catalyst definitions from built-in resources, external datapacks, and KubeJS alike while same-registry forward references remain valid. KubeJS `hideIfPresent(...)` references are additionally checked after every catalyst from the startup event is collected, allowing missing references in the mod namespace or a namespace owned by that startup event to produce a sourced KubeJS error and suppress the invalid generated resource. References to other namespaces remain deferred to authoritative registry decoding so catalysts supplied by external datapacks are not rejected before those datapacks load.
* Core behavior: final KubeJS resource and lang assembly is centralized in `RNSKubeJSAssembler`, while `RNSKubeJSPlugin` remains a thin KubeJS integration adapter that registers entrypoints and delegates to the assembler.
* Maintenance invariant: KubeJS builder emission should stay testable without booting the full KubeJS runtime. The `RNSKubeJSBuilderTest` GameTest harness therefore creates the KubeJS event objects and assembler in memory, then asserts on assembled resources after builder configuration.
* Maintenance invariant: custom catalyst, deposit-structure, structure-set, and mining-recipe/yield builder tests should pin method-level emission behavior, including default-field omission, overwrite semantics, and validation failures, so refactors cannot silently change the generated JSON surface.
* Core behavior: because the mining recipe surface now lives in KubeJS' normal server-side recipe event, it participates in `/reload` the same way as other KubeJS-authored recipes.
* Core behavior: the KubeJS plugin aggregates all KubeJS-authored deposit structures into one emitted `#create_rns:deposits` structure tag so scanner/admin flows continue to treat them as deposits without requiring a separate manual tag file.
* Core behavior: `StartupEvents.rnsEnableDeposits(...)` exposes two optional builders, `event.overworld()` and `event.nether()`. Each set selects deposits with overloaded `.deposit(...)` calls that make the deposit scannable in that dimension and may independently opt it into worldgen. The same builder also owns the random-spread `spacing`, `separation`, and `salt` overrides for the deposits that do generate. For KubeJS-authored custom deposit structures, inclusion in the overworld builder assigns overworld placement metadata, while inclusion in the nether builder assigns nether placement metadata.
* Core behavior: once a structure-set builder is requested for a dimension, KubeJS takes ownership of built-in deposit enablement for that dimension only. Selected structure ids become the source of truth for built-in deposit worldgen, built-in compat-recipe emission, and built-in deposit-block visibility tied to that dimension, while omitted dimensions continue using their built-in defaults. The depleted deposit block stays available even when no structures are selected.
* Core behavior: `RNSKubeJSAssembler` is the central authority for that structure-set ownership state. It captures one startup-event snapshot per KubeJS cache lifecycle, caches the selected structure ids plus derived enabled deposit blocks, and exposes those derived sets to always-loaded runtime code through a KubeJS-free bridge. Data, language, and deposit-selection assembly share that snapshot so startup callbacks and their errors are not replayed independently.
* Core behavior: invalid startup builder calls are reported through KubeJS with the offending method line when script source is available. Deferred validation failures, such as an omitted catalyst requirement or required deposit structure field, point to the corresponding `create(...)`, `tweak(...)`, `overworld()`, or `nether()` chain. Repeated `.deposit(...)` calls use their parent `overworld()` or `nether()` line because Rhino exposes only the enclosing chain location and cannot reliably distinguish an individual repeated call. Invalid builders or selections are excluded from generated resources, allowing startup to finish with normal KubeJS script errors instead of failing world datapack loading; direct Java builder calls retain their exception-based validation for the in-memory test harness. Direct item, block, fluid, sound, and attachment ids are checked against their startup registries where the builder schema requires an existing value. Item-tag syntax is checked immediately, but tag existence and contents remain deferred because datapack tags are not populated during startup script execution.
* System interaction: the feature is inert unless KubeJS is present; discovery is delegated to KubeJS through `kubejs.plugins.txt`, and mod metadata declares KubeJS as an optional both-side dependency for load ordering.
* System interaction: if `StartupEvents.rnsEnableDeposits(...)` has no listeners, built-in compat auto-detection and the default structure sets remain in control. When only one dimension builder is requested, that dimension's structure set and built-in `deposit_spec` scannability are overridden while the other dimension keeps its built-in defaults. A requested builder with no selected deposits still emits an explicit empty structure-set file for that dimension.
* System interaction: when that KubeJS structure-set event is used, the plugin also rewrites every built-in `deposit_spec` entry with an explicit `scannable` flag. Selected deposits stay scannable, while omitted built-in deposits are forced to `scannable: false` so the mod-shipped defaults cannot leak back into the scanner wheel.
* System interaction: a KubeJS-authored custom deposit structure cannot be assigned to both built-in dimension sets at once; that is treated as an invalid configuration because the generated structure/spec metadata would become ambiguous.
* Data and assets: block definitions still come from KubeJS block startup scripts, mining recipes now come from KubeJS server recipe scripts and serialize through the normal `create_rns:mining` recipe codec, catalyst registry JSON plus optional catalyst lang keys and KubeJS-authored miner-attachment tag entries come from the catalyst startup event, and deposit specs, structures, processors, structure lang, structure tags, and optional structure sets come from the separate deposit-structure startup event plus KubeJS-generated data.
* Maintenance invariant: `create_rns:deposit` should keep instantiating `DepositBlock` so future deposit-only behaviors continue to apply to KubeJS-authored deposits automatically.
* Maintenance invariant: KubeJS deposit-structure ids are now the authoritative identity for generated structure, deposit-spec, and structure-set entries, so multiple structure ids must be able to target the same deposit block without collisions.
* Maintenance invariant: KubeJS mining recipe authoring should remain on the server-side recipe event so `/reload` continues to rebuild those recipes from script changes.
* Maintenance invariant: KubeJS catalyst ids must stay identical to the emitted datapack registry ids so recipe catalyst references, hide-if-present rules, goggles, and JEI all continue to resolve against the same live registry entries.
...
## Miner Claim and Target State
* Player perspective: miners mine exactly the deposit blocks they claim. With Sable installed, miners in the same operating space compete normally; cross-sublevel discovery is currently diagnostic only, but the intended completed behavior is for a miner to claim and exclusively mine one remote target space rather than use unclaimed shared targets.
* Core behavior: `IDepositBlockOperator` owns deposit geometry and vein discovery, while `IDepositBlockClaimer` adds exclusive-ownership state and conflict filtering. `ClaimingBehaviour` owns the authoritative claimed target group; `MiningBehaviour` builds a `MiningProcess` directly from its positions, initializes durability, and advances and collects that process.
* Core behavior: an internal immutable claim groups a local/cross-sublevel flag and positions from exactly one operating space. There is no separate cached operating-space identity, active-target, or operating-selection state.
* Core behavior: without Sable, each dimension is one main space. With Sable, a containing sublevel's stable UUID identifies the space and positions outside a sublevel retain the dimension-specific main-space identity. Sable sublevels share their parent `Level`, so mining processes continue using the miner's level while the adapter resolves the target space from claim positions when needed.
* Lifecycle behavior: a claim replacement invalidates the mining process and initializes durability for the new positions. Disassembly releases the claim and clears the process. Client reads lazily reconstruct the process from the synchronized claim; an ordinary server read on an initialized miner retains an equal claim without rescanning deposits.
* Lifecycle behavior: local mining does not poll or cache a separate source-space identity. The bearing and effective mine-head tip are required to occupy the same operating space, so fresh local claims resolve their target identity from the bearing position. When Sable movement recreates a bearing block entity with copied source NBT, the restored local claim is validated once on the next tick against a representative target position. A same-space claim is retained, while a changed-space or empty claim is rebuilt; process reconstruction waits for this validation and restores compatible progress by deposit recipe.
* Lifecycle behavior: as a temporary cross-sublevel diagnostic, an assembled miner polls its mine head's detection volume along the operating direction each server tick. Chat markers report when a tagged deposit position in another operating space enters or leaves that volume; losing the assembled mine-head context clears the diagnostic state without changing claims.
* Player perspective: an operator can run `/rns debug deposit_detection true` to draw nearby mine-head detection volumes as cyan wireframe boxes for that invoking player. Running the command with `false` removes them immediately.
* Edge behavior: deposit-detection visualization is disabled by default, is not persisted, and resets when the client disconnects. The client-only command is available to local players without requiring server operator permissions.
* System interaction: operating-space identity is resolved from the effective mine-head tip. Bearing positions remain stable holder keys used only to find the owning block entity. Point-based holder queries and incremental client outlines use their relevant mine-head, deposit, or player position for dynamic same-space filtering; area-only reclaim queries use a representative bounding-box position under the invariant that the complete operating box belongs to one operating space.
* System interaction: Sable contact discovery follows its Create block-breaker compatibility model: the detection volume is transformed from the mine head's source sublevel into global coordinates, intersecting sublevels are queried, and the volume is inverse-transformed into each target space before tagged, loaded deposit blocks are collected. The source space is excluded because existing local claiming already owns that path.
* System interaction: the dedicated detection outline renderer uses Create's AABB Outliner in each mine head's local coordinates. Sable's Create outline compatibility applies the containing sublevel pose, so boxes remain attached to moving mine heads. Claim-outline wrench/target activation is independent and does not control detection boxes.
* System interaction: continuous mining sound range filtering and nearest-miner selection use operating-space adapter distances. Vanilla retains normal `BlockPos` distances, while Sable compares physically projected positions; playback keeps plot-space coordinates so Sable can attach sounds to moving sublevels.
* System interaction: client claim outlines partition nearby claims by operating space and submit one Create Outliner cluster per space. This preserves the normal combined outline within a space while allowing Sable to apply each sublevel's pose independently when several moving spaces are visible.
* Data and assets: the state model is code-only. Disk and client block-entity NBT store claimed positions plus process data through the existing claimer/process tags. Current local claims derive their operating-space metadata from runtime context after load; cross-sublevel claiming will require persisting or rediscovering the target-space metadata atomically with its positions.
* Data and assets: the diagnostic contact scan and literal chat markers are code-only and temporary; they add no datapack fields, generated assets, or persistent state.
* Data and assets: the debug command updates the client renderer directly without a network payload; no server attachment or client configuration persists the setting.
* Maintenance invariant: a `null` claim means claiming has not completed or has been released; an empty claim means claiming completed without finding an operable deposit. Mining processes, effects, tooltips, serialization, and outlines consume that same authoritative claim.
* Maintenance invariant: claimed targets always belong to exactly one operating space. Common operating code depends only on the mod-owned operating-space adapter and value types; Sable Companion API types remain confined to the guarded optional adapter, and loading without Sable must select the vanilla adapter without resolving those types.
* Maintenance invariant: each `MineHeadSize` defines its detection radius, axial length, and forward center offset. Radius is applied on the two axes perpendicular to the operating direction, while half the length is applied on the operating axis.
* Maintenance invariant: transformed contact discovery and client visualization must consume the same constructed detection AABB so the displayed box cannot disagree with enter/leave detection.
* Known limitation: transformed cross-sublevel contact discovery does not yet construct exclusive claims or mining processes. Target-space conflict filtering, metadata persistence/rediscovery, and movement-driven refresh must be completed before enabling that gameplay path.
...
* System interaction: client scanner icon selection is now derived locally from loaded `deposit_spec` entries that are both `scannable` and present in the active `#create_rns:deposits` structure tag for the current dimension. KubeJS can therefore make a deposit scannable without also enabling its worldgen, while omitted deposits disappear from the scanner wheel even if their built-in datapack specs still exist.
* System interaction: `/rns scanner` command structure suggestions now rely on the same `#create_rns:deposits` tag rather than on active worldgen placements alone, so scan-only deposits remain valid command targets for manual admin-added locations while `locate all` still reports only deposits that actually have placements.
* Data and assets: target types are data-driven via `deposit_spec` datapack entries (`scanner_icon_item`, `map_icon_item`, `structure`, and optional `scannable`), not hardcoded. Both icon fields use the shared ordered item-fallback format, so each one may be authored as either a single item id or a fallback list containing item ids and item tags.
...
* Maintenance invariant: the client-side scanner wheel must only expose icons whose mapped deposit structures are present in the active `#create_rns:deposits` tag for that dimension, otherwise deposit specs for intentionally disabled structures can still become selectable and desync scanner behavior from the configured scanner/worldgen surface.
* Known limitation: request handling assumes selected scanner icon maps to a valid tagged deposit spec; invalid or conflicting scanner-icon specs are still not defensively handled beyond the existing registry-build validation.
...
* Core behavior: the codec hardcodes the shared deposit defaults (underground-ores step, no terrain adaptation, rigid single-piece placement, standard liquid settings), while deposit JSON still controls biome scope, weighted template selection, and placement through `placement_strategy` plus `height`.
...
* Core behavior: `height` is now sampled as a Y offset rather than a literal start height. For `overworld`, the offset is applied relative to the `OCEAN_FLOOR_WG` first-free height at the chosen piece center.
* Core behavior: the current Nether placement helper scans the structure-piece center column for the first run of 4 consecutive air blocks, treats that cavity floor as the reference surface, then applies the sampled `height` offset to the buried piece bottom. The cavity is accepted only if at least 70% of the 1-block-thick shell around the candidate piece volume (floor, ceiling, and four walls) is tagged as `minecraft:nether_carver_replaceables`.
...
* Data and assets: deposit structure JSON now uses the mod-owned `create_rns:deposit` schema rather than vanilla jigsaw's full field set; the structure implementation translates `placement_strategy` and `height` into terrain-relative placement offsets, then resolves the selected template from the structure's own weighted `structures` list instead of a separate template-pool start file.
...
* Maintenance invariant: `height` remains a terrain-relative Y offset for both placement strategies; overworld offsets are applied relative to projected terrain height, while Nether offsets are applied relative to the first qualifying cavity floor found by the center-column scan.
* Maintenance invariant: overworld strategy must continue to add `OCEAN_FLOOR_WG` projection, while the current nether strategy must continue to require a 4-block air cavity plus a 70% supporting shell of `minecraft:nether_carver_replaceables`, unless deposit worldgen docs and generated defaults are updated together.
...
* Edge behavior: malformed values in optional mining-recipe, yield, weighted-item, catalyst-set, deposit-spec, and deposit-structure fields now fail datapack decoding instead of silently falling back to defaults when the field is present.
...
* System interaction: when KubeJS structure-set ownership is active, built-in mining-recipe emission is filtered by the deposit blocks referenced by the selected structure ids instead of by compat auto-detection alone.
...
* Edge behavior: fallback entries are not considered resolved at raw codec-parse time, but a required group containing only direct item ids now fails decoding when none is registered. Accepted direct ids and all tag-backed entries stay pending until live registry resolution runs.
* Edge behavior: strict late fallback resolution logs an error line for every attempted candidate that fails, then the owning weighted item or yield is discarded if nothing resolves. Compat fallback groups may resolve to nothing without producing a recipe decode error.
...
* Core behavior: deposit specs now also carry a `scannable` flag that defaults to `true`; scanner-selection code only exposes specs marked scannable, while other lookups such as map-icon resolution can still read the spec metadata.
...
* System interaction: when KubeJS structure-set ownership is active, the KubeJS-generated override pack rewrites every built-in deposit spec with an explicit `scannable` flag derived from the selected structure ids, allowing compat deposit structures to stay selectable even when the default compat auto-detect path would have omitted them.
