...
* System interaction: if `StartupEvents.createRnsStructureSet(...)` has no listeners, structure-set files are still left to normal datapack authoring. If listeners are present, the KubeJS plugin emits full replacement `create_rns:deposits` structure tags plus `create_rns:deposits` and `create_rns:nether_deposits` structure-set files from those two fixed builders, so startup scripts become the authoritative scanner-visibility and deposit-worldgen selection surface for that pack.
