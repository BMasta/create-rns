...
* Core behavior: `IDepositBlockOperator` owns deposit geometry and vein discovery, while `IDepositBlockClaimer` adds exclusive-ownership state and conflict filtering. `HybridOperatingBehaviour` owns claim and active-selection transitions; `MiningBehaviour` turns the active selection into a `MiningProcess`, initializes durability, and advances and collects that process.
* Core behavior: exclusive claim ownership and active targets are separate state. Active positions are stored inside one immutable `OperatingSelection` together with its logical operating-space identity and local/remote mode. In current local mining, every successful claim refresh creates a selection whose positions are an equal defensive copy of the claim.
* Core behavior: without Sable, each dimension is one main space. With Sable, a containing sublevel's stable UUID identifies the space and positions outside a sublevel retain the dimension-specific main-space identity. Sable sublevels share their parent `Level`, so candidate and selection state carries a dimension key and logical-space identity rather than a separate target `Level`. Only local-exclusive mining mode is reachable at this stage; the Sable adapter's remote deposit results are diagnostic contacts and do not become claims or active targets.
* Core behavior: mining-process construction, load validation, durability initialization, remaining-use display, rates, and effects use `OperatingSelection.positions` rather than reading exclusive claims directly.
* Lifecycle behavior: a claim refresh replaces claim and selection together, then invokes the mining-specific selection-change hook. That hook rebuilds the process and initializes durability. Disassembly clears the selection, claim, and process together.
* Lifecycle behavior: while a local selection exists, the server performs a constant-time space-identity lookup before mining advances. If sublevel reconstruction or equipment changes move the effective mine-head tip to another logical space, stale targets are cleared and local claims are recomputed; this lookup does not scan deposits or other sublevels.
...
* Data and assets: the state model is code-only. Disk NBT stores the exclusive claim and process data but not `OperatingSelection`; a local selection is reconstructed from the claim and current mine-head context after load. Transient server-to-client block-entity packets serialize the complete selection so client processes, effects, and tooltips receive mode, space identity, and positions atomically.
...
* Maintenance invariant: in local mode, claim and selection positions must remain equal without sharing the same set instance. A `null` selection means target selection has not completed; an empty position set means selection completed without finding an operable deposit.
* Maintenance invariant: claim serialization and claim outlines remain ownership-only. Transient client presentation may consume active targets, but active-target state must not become persistent disk ownership.
* Maintenance invariant: active targets always belong to exactly one selected target group. Common operating code depends only on the mod-owned operating-space adapter and value types; Sable Companion API types remain confined to the guarded optional adapter, and loading without Sable must select the vanilla adapter without resolving those types.
...
* Known limitation: the transition from chat diagnostics to exclusive cross-sublevel claiming remains intentionally undecided.
