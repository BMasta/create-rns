...
* Core behavior: deposit mining recipes may now be authored from `ServerEvents.recipes(...)` through a custom `create_rns:mining` recipe schema that exposes callback-style nested yield builders mirroring the code-side `MiningRecipeBuilder` and `YieldBuilder` flow. The mined deposit block is configured explicitly through `.block(...)`, keeping it separate from the recipe id; it must still belong to `#create_rns:deposit_blocks` to work as a deposit, but Forge 1.20 recipe decoding cannot validate current tag membership because tags are not yet bound. Catalyst ids decode as namespaced resource locations and resolve into registry holders during recipe initialization, when `RegistryAccess` is available. Yield slot colors may be authored either as raw ARGB integers or as `#rrggbb` shorthand strings. Required `.item(...)` candidate groups containing only direct item ids are validated by the mining recipe codec against the completed item registry; if none exists, KubeJS reports and skips the individual recipe without failing the datapack reload, while `.compatItem(...)` remains optional and tag-containing groups remain deferred until live tag resolution. Malformed block and catalyst values, invalid durability, out-of-range chance values, and non-positive item weights are retained for normal recipe decoding rather than throwing from the event callback. This preserves the authoritative mining codec's error messages and prevents secondary errors from partially built recipes.
